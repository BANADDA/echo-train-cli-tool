...
const executeScript = async (docId) => {
    ...
    try {
        ...
        // Install packages with non-interactive options and upgraded pip if needed
        await executeCommand(
            `${venvPath}${pythonExecutable} -m pip install --upgrade pip`,
            "Upgrading pip"
        );
        await executeCommand(
            `${venvPath}${pythonExecutable} -m pip install -q -r "${requirementsPath}" --disable-pip-version-check`,
            "Installing packages"
        );
        ...
        const output = await executeCommand(
            `${venvPath}${pythonExecutable} "${scriptPath}" "${resultsDir}"`,
            "Running the Python script"
        );
        
        // Move all files generated by the script to the results directory
        const scriptOutputDir = path.join(jobDir, 'output');
        fs.ensureDirSync(scriptOutputDir);

        const files = fs.readdirSync(scriptOutputDir); // Get list of files generated by the script
        files.forEach(file => {
            const sourcePath = path.join(scriptOutputDir, file); // Source path of the file
            const destPath = path.join(resultsDir, file); // Destination path in the results directory
            fs.moveSync(sourcePath, destPath); // Move the file to the results directory
        });

        console.log(`All output files saved to ${resultsDir}`);
    } catch (error) {
